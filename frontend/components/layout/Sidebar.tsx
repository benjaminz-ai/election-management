...
import {
  LayoutDashboard, Users, UsersRound, UserCheck,
  Shield, Search, Tag, PhoneCall, MessageSquareMore,
  UserCog, LogOut, Snowflake, BarChart3, X,
} from "lucide-react";
  ...
  return (
    <>
      ...
      <aside className={`sidebar${isOpen ? " sidebar-open" : ""}`}>
        ...
        <div style={{ padding: "20px 16px 16px", borderBottom: "1px solid rgba(255,255,255,0.08)", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#209dd7,#753991)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <Shield size={18} color="#fff" />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ color: "#fff", fontWeight: 700, fontSize: 15, lineHeight: 1.2 }}>מערכת ניהול</div>
            <div style={{ color: "#64748b", fontSize: 11 }}>בחירות</div>
          </div>
          {onClose && (
            <button
              onClick={onClose}
        ...
        {currentUser && (
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", padding: "14px 16px" }}>
            ...
            <button onClick={handleLogout} style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.18)", borderRadius: 8, color: "#f87171", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
              <LogOut size={14} />
              יציאה מהמערכת
            </button>
          </div>
