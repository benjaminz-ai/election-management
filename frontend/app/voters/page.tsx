...
export default function VotersPage() {
  ...
  const openEdit = (v: Voter) => { setForm({ ...v, address: { ...v.address }, subGroupIds: v.subGroupIds ?? [] }); setEditing(v); setDupError(null); setShowForm(true); };
  ...
  const toggleGroup = (gid: string) => {
    setForm((f) => {
      ...
      const removedSubs = removing ? (f.subGroupIds ?? []).filter(sid => { const sg = subGroups.find(x => x.id === sid); return sg?.parentGroupId === gid; }) : [];
      return { ...f, groupIds: removing ? f.groupIds.filter(id => id !== gid) : [...f.groupIds, gid], subGroupIds: removing ? (f.subGroupIds ?? []).filter(sid => !removedSubs.includes(sid)) : f.subGroupIds ?? [] };
    });
  };

  const toggleSubGroup = (sgid: string, parentGroupId: string) => {
    setForm((f) => {
      const subs = f.subGroupIds ?? [];
      const removing = subs.includes(sgid);
      const newGroupIds = !removing && !f.groupIds.includes(parentGroupId) ? [...f.groupIds, parentGroupId] : f.groupIds;
      return { ...f, groupIds: newGroupIds, subGroupIds: removing ? subs.filter(id => id !== sgid) : [...subs, sgid] };
    });
  };
      ...
      case "groups": return voterGroups.length === 0
        ...
        : <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            {voterGroups.slice(0, 2).map(g => <span key={g.id} className="badge badge-blue">{g.name}</span>)}
                    {subGroups.filter(sg => (v.subGroupIds ?? []).includes(sg.id)).map(sg => (
                      <span key={sg.id} style={{ fontSize: 11, padding: "2px 7px", borderRadius: 10, background: "rgba(139,92,246,0.1)", color: "#8b5cf6", fontWeight: 600 }}>↳ {sg.name}</span>
                    ))}
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: 14 }}>
                <label className="label" style={{ marginBottom: 8 }}>שיוך לקבוצות ותת-קבוצות</label>
                            ...
                            {groupSubGroups.map(sg => {
                              ...
                              return (
                                <button key={sg.id} type="button" onClick={() => toggleSubGroup(sg.id, g.id)}
                                  style={{ padding: "3px 10px", borderRadius: 20, border: sgSel ? "1px solid #8b5cf6" : "1px dashed var(--border)", background: sgSel ? "rgba(139,92,246,0.1)" : "#fff", color: sgSel ? "#8b5cf6" : "var(--gray-text)", fontWeight: sgSel ? 600 : 400, fontSize: 11, cursor: "pointer" }}>
                                  ↳ {sg.name}
                                </button>
                              );
                            })}
