...
import { Search, MapPin, UserCheck, UsersRound, X, Hash, User, Building2 } from "lucide-react";
    ...
    switch (mode) {
      ...
      case "streetAndNumber":
        return voters.filter((v) =>
          `${v.address.street} ${v.address.streetNumber}`.toLowerCase().includes(q)
        );
      case "groupLeader": {
        const matched = groupLeaders.filter((gl) =>
    ...
    <div>
      ...
      <div className="card" style={{ padding: 20, marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
          {MODES.map((m) => (
            <button
              key={m.key}
              onClick={() => { setMode(m.key); setQuery(""); }}
              style={{
                display: "flex", alignItems: "center", gap: 6,
                padding: "7px 14px", borderRadius: 8, fontSize: 13, fontWeight: 600,
                cursor: "pointer", border: "1.5px solid",
                borderColor: mode === m.key ? "var(--blue-primary)" : "var(--border)",
                background: mode === m.key ? "rgba(32,157,215,0.08)" : "#fff",
                color: mode === m.key ? "var(--blue-primary)" : "var(--text-secondary)",
      ...
      {!query.trim() && (
        <div className="empty-state" style={{ marginTop: 40 }}>
          <div className="empty-state-icon">
            <Search size={28} color="var(--blue-primary)" />
          </div>
          ...
          <p style={{ margin: 0, color: "var(--gray-text)", fontSize: 14 }}>
            בחר קטגורית חיפוש והכנס ערך לחיפוש
          </p>
        </div>
      )}
      ...
      {query.trim() && results.length === 0 && (
        <div className="empty-state">
          ...
          <p style={{ margin: 0, color: "var(--gray-text)", fontSize: 14 }}>
            לא נמצאו בוחרים עבור "{query}"
          </p>
        </div>
      )}

      {/* Grouped by building (streetAndNumber mode) */}
          ...
          {Object.entries(visibleGroupedResults).map(([buildingKey, buildingVoters]) => (
            <div key={buildingKey} className="card" style={{ overflow: "hidden", padding: 0 }}>
              <div style={{
                ...
                <span style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, marginRight: "auto" }}>
                  {buildingVoters.length} בוחרים
                </span>
              </div>
                    ...
                    return (
                      <tr key={v.id} style={{ borderBottom: "1px solid var(--border)" }}
                        ...
                        <td style={tdStyle}>
                          ...
                          {v.address.apartment && (
                            <div style={{ fontSize: 11, color: "var(--gray-text)" }}>דירה {v.address.apartment}</div>
                          )}
                        </td>
                        <td style={tdStyle}>
                          {st && (
                        ...
                        <td style={tdStyle}>
                          {leader ? (
                            <span style={{ fontSize: 12, color: "var(--purple-secondary)", fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
                              <UserCheck size={11} />{leader.firstName} {leader.lastName}
                            </span>
                          ) : (
                ...
                return (
                  <tr key={v.id} style={{ borderBottom: "1px solid var(--border)" }}
                    ...
                    <td style={tdStyle}>
                      {st ? (
                        <span style={{
                          display: "inline-block", fontSize: 12, padding: "3px 9px",
                          borderRadius: 20, fontWeight: 600,
                          background: st.color + "22", color: st.color, border: `1px solid ${st.color}55`
                        }}>{st.name}</span>
                      ) : (
                    ...
                    <td style={tdStyle} className="hide-mobile">
                      {leader ? (
                        <span style={{ fontSize: 13, color: "var(--purple-secondary)", fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
                          <UserCheck size={12} />{leader.firstName} {leader.lastName}
                        </span>
                      ) : (
                        <span className="badge badge-gray">לא משויך</span>
                      )}
