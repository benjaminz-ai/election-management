...
export default function GroupsPage() {
  ...
  const toggleExpand = (gid: string) => {
    setExpandedGroups((prev) => { const next = new Set(prev); next.has(gid) ? next.delete(gid) : next.add(gid); return next; });
  };

  const openAdd = () => { setForm(emptyGroup()); setEditing(null); setShowForm(true); setOriginalName(""); };
  const openEdit = (g: Group) => { setForm({ name: g.name, groupLeaderId: g.groupLeaderId, voterIds: g.voterIds, subGroupIds: g.subGroupIds ?? [] }); setEditing(g); setOriginalName(g.name); setShowForm(true); };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.groupLeaderId && !editing) return;
    if (editing) {
      if (form.name !== originalName) { setRenameConfirm({ group: editing, newName: form.name }); setShowForm(false); return; }
      updateGroup({ ...editing, ...form }); setShowForm(false);
    } else { addGroup({ ...form, id: generateId() }); setShowForm(false); }
  };

  const confirmRename = () => {
    if (renameConfirm) { updateGroup({ ...renameConfirm.group, name: renameConfirm.newName, groupLeaderId: form.groupLeaderId, voterIds: form.voterIds }); setRenameConfirm(null); }
  };
  const confirmDelete = () => { if (deleteTarget) { deleteGroup(deleteTarget.id); setDeleteTarget(null); } };

  const openAddSubGroup = (parentGroupId: string) => {
    setEditingSubGroup(null); setSubFormParentGroupId(parentGroupId); setSubFormName(""); setShowSubForm(true);
    setExpandedGroups((prev) => new Set([...prev, parentGroupId]));
  };
  const openEditSubGroup = (sg: SubGroup) => { setEditingSubGroup(sg); setSubFormParentGroupId(sg.parentGroupId); setSubFormName(sg.name); setShowSubForm(true); };

  const handleSubSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!subFormName.trim()) return;
    if (editingSubGroup) {
      if (subFormName !== editingSubGroup.name) { setSubRenameConfirm({ sg: editingSubGroup, newName: subFormName }); setShowSubForm(false); return; }
      updateSubGroup({ ...editingSubGroup, name: subFormName }); setShowSubForm(false);
    } else { addSubGroup({ id: generateId(), name: subFormName, parentGroupId: subFormParentGroupId, voterIds: [] }); setShowSubForm(false); }
  };

  const confirmSubRename = () => { if (subRenameConfirm) { updateSubGroup({ ...subRenameConfirm.sg, name: subRenameConfirm.newName }); setSubRenameConfirm(null); } };
  const confirmDeleteSubGroup = () => { if (deleteSubTarget) { deleteSubGroup(deleteSubTarget.id); setDeleteSubTarget(null); } };

  return (
    <div>
      <PageHeader title="קבוצות" subtitle={`${groups.length} קבוצות`} action={
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 }}>
        {visibleGroups.map((g) => {
          ...
          return (
            <div key={g.id} className="card" style={{ padding: 20, borderTop: `3px solid ${isOrphan ? "#ef4444" : "var(--blue-primary)"}` }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 15, color: "var(--dark-navy)", marginBottom: 4 }}>{g.name}</div>
                  {isOrphan ? (
                    <span className="badge badge-red" style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><AlertCircle size={11} />ללא ראש קבוצה</span>
                  ) : (
                    <span className="badge badge-purple">{gl?.firstName} {gl?.lastName}</span>
                  )}
                </div>
              ...
              {groupVoters.length > 0 && (
                <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 4 }}>
                  {groupVoters.slice(0, 4).map((v) => <span key={v.id} className="badge badge-gray">{v.firstName} {v.lastName}</span>)}
                  {groupVoters.length > 4 && <span className="badge badge-gray">+{groupVoters.length - 4}</span>}
                </div>
              )}

              <div style={{ marginTop: 14, borderTop: "1px solid var(--border)", paddingTop: 12 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                  <button type="button" onClick={() => toggleExpand(g.id)}
                    ...
                    <FolderTree size={13} />תת-קבוצות ({groupSubGroups.length})
                  </button>
                  <button type="button" onClick={() => openAddSubGroup(g.id)}
                    ...
                    {groupSubGroups.length === 0 ? (
                      ...
                      <div key={sg.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(32,157,215,0.05)", borderRadius: 8, padding: "7px 10px" }}>
                        <div>
                          ...
                          <span style={{ fontSize: 11, color: "var(--gray-text)", marginRight: 8 }}>{sg.voterIds.length} בוחרים</span>
                        </div>
      ...
      {groups.length === 0 && <div className="card" style={{ padding: 48, textAlign: "center", color: "var(--gray-text)" }}>אין קבוצות. לחץ "הוסף קבוצה" כדי להתחיל.</div>}

      {showForm && (
        <div className="modal-overlay" onClick={() => setShowForm(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div style={{ marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(32,157,215,0.1)", display: "flex", alignItems: "center", justifyContent: "center" }}><UsersRound size={17} color="var(--blue-primary)" /></div>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: "var(--dark-navy)" }}>{editing ? "עריכת קבוצה" : "הוספת קבוצה"}</h2>
            </div>
      ...
      {showSubForm && (
        <div className="modal-overlay" onClick={() => setShowSubForm(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 420 }}>
            <div style={{ marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(32,157,215,0.1)", display: "flex", alignItems: "center", justifyContent: "center" }}><FolderTree size={17} color="var(--blue-primary)" /></div>
              <div>
      ...
      {renameConfirm && <ConfirmDialog title="שינוי שם קבוצה" message={`שינוי שם הקבוצה מ-"${renameConfirm.group.name}" ל-"${renameConfirm.newName}" יתעדכן בכל הרשומות. פעולה זו בלתי הפיכה.`} confirmLabel="אשר שינוי שם" onConfirm={confirmRename} onCancel={() => setRenameConfirm(null)} danger={false} />}
      {deleteTarget && <ConfirmDialog title="מחיקת קבוצה" message={`האם למחוק את הקבוצה "${deleteTarget.name}"? כל תת-הקבוצות יימחקו גם כן. פעולה זו בלתי הפיכה.`} confirmLabel="מחק קבוצה" onConfirm={confirmDelete} onCancel={() => setDeleteTarget(null)} />}
      ...
      {deleteSubTarget && <ConfirmDialog title="מחיקת תת-קבוצה" message={`האם למחוק את תת-הקבוצה "${deleteSubTarget.name}"? הבוחרים לא יימחקו, אך השיוך יוסר. פעולה זו בלתי הפיכה.`} confirmLabel="מחק תת-קבוצה" onConfirm={confirmDeleteSubGroup} onCancel={() => setDeleteSubTarget(null)} />}
    </div>
  );
}
