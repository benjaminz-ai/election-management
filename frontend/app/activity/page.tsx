...
export default function ActivityPage() {
  ...
  return (
    <div>
      ...
      {/* Visual summary: rep ranking by call volume, split by call outcome */}
      {loaded && !error && totals.calls > 0 && (
        <RepRanking reps={sortedAggs} callStatuses={callStatuses} userById={userById} />
      )}
...
// Visual summary: a horizontal ranking of reps by call volume. Each bar's
// length is proportional to that rep's calls vs. the top performer, and the
// bar is split by call outcome (using the company's call-status colors) so the
// manager sees both quantity and quality at a glance.
function RepRanking({ reps, callStatuses, userById }: {
  reps: Agg[];
  callStatuses: { id: string; name: string; color: string }[];
  userById: Map<string, AppUser>;
}) {
  const list = reps.filter((r) => r.total > 0);
  ...
  return (
    <div className="card" style={{ padding: "16px 18px", marginBottom: 22 }}>
      <div style={{ fontSize: 14, fontWeight: 700, color: "var(--dark-navy)", marginBottom: 14 }}>דירוג נציגים לפי שיחות</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {list.map((r) => (
          <div key={r.userId} style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 120, flexShrink: 0, fontSize: 13, fontWeight: 600, color: "var(--dark-navy)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", textAlign: "right" }}>{nameOf(r.userId)}</div>
            <div style={{ flex: 1, minWidth: 0, background: "#f1f5f9", borderRadius: 6, overflow: "hidden" }}>
              <div style={{ display: "flex", width: `${(r.total / max) * 100}%`, minWidth: 4, height: 20, borderRadius: 6, overflow: "hidden" }}>
                {callStatuses.map((c) => {
                  const n = r.byCall[c.id] || 0;
                  if (!n) return null;
                  return <div key={c.id} title={`${c.name}: ${n}`} style={{ width: `${(n / r.total) * 100}%`, background: c.color }} />;
                })}
              </div>
            </div>
            <div style={{ width: 34, flexShrink: 0, textAlign: "left", fontWeight: 800, color: "#209dd7", fontSize: 15 }}>{r.total}</div>
          </div>
        ))}
      </div>
    </div>
